...
from gepa.adapters.caching_adapter import My
import importlib
import struct
...
def cache_simulate(trace_path):
    result_dict = dict()
    trace = Trace(trace_path=trace_path, next_vtime_set=True)
    cap = max(int(trace.get_ndv() * 0.1), 1)
    cache = Cache(
        config=CacheConfig(
            capacity=cap, 
    ...
    for entry in trace.entries:
        obj = CacheObj(key=str(entry.key), size=entry.size, consider_obj_size=False)
        cache.get(obj)
    hit_rate = round(cache.hit_count / cache.access_count, 6)
    result_dict[os.path.basename(trace_path)] = hit_rate
    
    return result_dict

parser = argparse.ArgumentParser()
parser.add_argument('--result_path', type=str, required=True, help='Path to save the evaluation results')
parser.add_argument('--trace_path', type=str, required=False, help='Path to the trace file')
...
try:
    ...
    print(f"Error saved to {result_path}")